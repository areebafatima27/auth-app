...
import { getAuth, signOut } from 'firebase/auth'; // Updated import
...
const Dashboard = () => {
  ...
  const handleLogout = async () => {
    try {
      await signOut(auth);  // Use 'auth' initialized by 'getAuth'
      navigate('/');
    } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <aside className={`bg-purple-700 text-white w-64 space-y-6 py-7 px-2 absolute inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition duration-200 ease-in-out`}>
        <nav>
          <a href="#" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-purple-600 hover:text-white">
            Home
          </a>
          <a href="#" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-purple-600 hover:text-white">
            Profile
          ...
          <div className="container mx-auto">
            ...
            <div className="bg-white shadow-md rounded-lg p-6">
              <p className="text-gray-600">
                This is your personal dashboard. Here you can view your account information, manage your settings, and access various features of our application.
              </p>
              <div className="mt-4">
